...
xport_lookup:
    /* If the flow is from an explicit tunnel, we don't want it to be
     * recirculated and have another flow with the vport as its in-port.
     */
    if (flow->tunnel.flags & FLOW_TNL_F_EXPLICIT) {
...
reversible_actions(const struct ofpact *ofpacts, size_t ofpacts_len)
{
    ...
    OFPACT_FOR_EACH (a, ofpacts, ofpacts_len) {
        ...
        case OFPACT_ENCAP:
        case OFPACT_DECAP:
        case OFPACT_TUN_ENCAP:
        case OFPACT_TUN_DECAP:
...
static void
compose_tun_encap_action(struct xlate_ctx *ctx,
                         struct ofpact_tun_cap *tun_encap)
{
    struct netdev_tnl_build_header_params tnl_params;
    struct ovs_action_push_tnl tnl_push_data;
    struct flow *flow = &ctx->xin->flow;
    ...
    smac = flow->tunnel.eth_src;
    dmac = flow->tunnel.eth_dst;
    ...
    if (flow->tunnel.ip_dst) {
        in6_addr_set_mapped_ipv4(&d_ip6, flow->tunnel.ip_dst);
    } else {
        d_ip6 = flow->tunnel.ipv6_dst;
    }
    if (IN6_IS_ADDR_UNSPECIFIED(&d_ip6)) {
        xlate_report(ctx, OFT_WARN, "tnl_dst must be specified");
        return;
    }
    if (IN6_IS_ADDR_UNSPECIFIED(&s_ip6)) {
        s_ip6 = d_ip6;
    }

    netdev_init_tnl_build_header_params(&tnl_params, flow, &s_ip6, dmac, smac);
    xport = get_ofp_port(ctx->xbridge, tun_encap->tnl_port);
    if (!xport) {
        xlate_report(ctx, OFT_WARN, "Could not find tunnel %d",
                     tun_encap->tnl_port);
        return;
    }
    /* Explicit tunnels won't lookup their output port, which would have had
     * the side-effect of setting inherited fields. Do it now.
     */
    tnl_port_inherit(xport->ofport, flow, ctx->wc);
    tnl_push_data.tnl_type = OVS_VPORT_TYPE_UNSPEC;
    err = tnl_port_build_header(xport->ofport, &tnl_push_data, &tnl_params);
    if (err) {
        xlate_report(ctx, OFT_WARN, "native tunnel header build failed");
        return;
    }
    if (tnl_push_data.tnl_type == OVS_VPORT_TYPE_VXLAN) {
        flow->tunnel.type = TUN_TYPE_VXLAN;
    } else if (tnl_push_data.tnl_type == OVS_VPORT_TYPE_GRE) {
        flow->tunnel.type = TUN_TYPE_GRE;
    } else if (tnl_push_data.tnl_type == OVS_VPORT_TYPE_GENEVE) {
        flow->tunnel.type = TUN_TYPE_GENEVE;
    }
    tnl_push_data.tnl_port = xport->odp_port;
    tnl_push_data.out_port = ofp_port_to_odp_port(ctx->xbridge, OFPP_LOCAL);
...
static void
compose_tun_decap_action(struct xlate_ctx *ctx,
                         struct ofpact_tun_cap *tun_decap)
{
    ...
    odp_tnl_port = ofp_port_to_odp_port(ctx->xbridge, tun_decap->tnl_port);
    nl_msg_put_odp_port(ctx->odp_actions, OVS_ACTION_ATTR_TUN_DECAP,
                        odp_tnl_port);
    ctx->xin->flow.tunnel.type = TUN_TYPE_NONE;
}
                 ...
                 bool group_bucket_action)
{
    ...
    OFPACT_FOR_EACH (a, ofpacts, ofpacts_len) {
        ...
        case OFPACT_TUN_ENCAP:
            compose_tun_encap_action(ctx, ofpact_get_TUN_ENCAP(a));
            break;

        case OFPACT_TUN_DECAP:
            compose_tun_decap_action(ctx, ofpact_get_TUN_DECAP(a));
            break;
...
xlate_wc_finish(struct xlate_ctx *ctx)
{
    ...
    /* In case of explicit tunnel with no tunnel matches, clear tunnel mask
     * and set only the explicit flag.
     */
    if (ctx->wc->masks.tunnel.flags & FLOW_TNL_F_EXPLICIT &&
        ctx->wc->masks.tunnel.tun_id == 0 &&
        ctx->wc->masks.tunnel.ip_src == 0 &&
        ctx->wc->masks.tunnel.ip_dst == 0 &&
        ipv6_is_zero(&ctx->wc->masks.tunnel.ipv6_src) &&
        ipv6_is_zero(&ctx->wc->masks.tunnel.ipv6_dst)) {
        uint8_t type = ctx->wc->masks.tunnel.type;

        memset(&ctx->wc->masks.tunnel, 0, sizeof ctx->wc->masks.tunnel);
        ctx->wc->masks.tunnel.flags = FLOW_TNL_F_EXPLICIT;
        ctx->wc->masks.tunnel.type = type;
    }
}
