...
tnl_wc_init(struct flow *flow, struct flow_wildcards *wc)
{
    if (tnl_port_should_receive(flow)) {
        ...
        /* OpenFlow does not support the +csum flag on tunnels and
         * netdev offloads ignore that match. A packet in SW datapath
         * however would generate different dp-flows depending on
         * whether its csum is set, breaking some offload implementation.
         * Align all layers to ignore the csum in all matches, even
         * in SW. */
        wc->masks.tunnel.flags = (FLOW_TNL_F_DONT_FRAGMENT |
                                  /* FLOW_TNL_F_CSUM - See comment above */
                                  FLOW_TNL_F_KEY);
        if (flow->tunnel.flags & FLOW_TNL_F_EXPLICIT) {
            wc->masks.tunnel.flags |= FLOW_TNL_F_EXPLICIT;
        }
              ...
              struct flow_wildcards *wc) OVS_EXCLUDED(rwlock)
{
    ...
    if (!cfg->is_explicit && !cfg->ip_src_flow) {
    ...
    if (!cfg->is_explicit && !cfg->ip_dst_flow) {
    ...
    flow->tunnel.flags |= (cfg->dont_fragment ? FLOW_TNL_F_DONT_FRAGMENT : 0)
        ...
        | ((cfg->out_key_present | cfg->is_explicit) ? FLOW_TNL_F_KEY : 0);
