...
/* Actions similar to standard actions are listed with the standard actions. */
    ...
    /* NX1.0+(51): struct nx_action_tun_cap, ... */
    NXAST_RAW_TUN_ENCAP,

    /* NX1.0+(52): struct nx_action_tun_cap, ... */
    NXAST_RAW_TUN_DECAP,
...
/* The action structure for TUN_ENCAP/TUN_DECAP is "struct ext_action_header",
 * followed by the tnl_port property.
 */
struct nx_action_tun_cap {
    ovs_be16 type;                  /* OFPAT_VENDOR. */
    ovs_be16 len;                   /* Length is 16. */
    ovs_be32 vendor;                /* NX_VENDOR_ID. */
    ovs_be16 subtype;               /* TUN_ENCAP/TUN_DECAP. */

    ovs_be16 tnl_port;
    uint8_t pad[4];
};
OFP_ASSERT(sizeof(struct nx_action_tun_cap) == 16);

static enum ofperr
decode_NXAST_RAW_TUN_ENCAP(const struct nx_action_tun_cap *nate,
                           enum ofp_version ofp_version OVS_UNUSED,
                           struct ofpbuf *ofpacts)
{
    struct ofpact_tun_cap *tun_encap;

    tun_encap = ofpact_put_TUN_ENCAP(ofpacts);
    tun_encap->tnl_port = nate->tnl_port;
    return 0;
}

static void
encode_TUN_ENCAP(const struct ofpact_tun_cap *tun_encap,
                 enum ofp_version ofp_version OVS_UNUSED, struct ofpbuf *out)
{
    struct nx_action_tun_cap *nate = put_NXAST_TUN_ENCAP(out);

    nate->tnl_port = tun_encap->tnl_port;
}

static char * OVS_WARN_UNUSED_RESULT
parse_TUN_ENCAP(char *arg, const struct ofpact_parse_params *pp)
{
    struct ofpact_tun_cap *tun_encap;
    char *tnl_port_s = arg;

    tun_encap = ofpact_put_TUN_ENCAP(pp->ofpacts);

    if (tnl_port_s && tnl_port_s[0]) {
        if (!ofputil_port_from_string(tnl_port_s, pp->port_map,
                                      &tun_encap->tnl_port)) {
            return xasprintf("%s: tun_encap tnl_port to unknown port",
                             tnl_port_s);
        }
    } else {
        tun_encap->tnl_port = OFPP_IN_PORT;
    }
...
static void
format_TUN_ENCAP(const struct ofpact_tun_cap *a,
                 const struct ofpact_format_params *fp)
{
    ds_put_format(fp->s, "%stun_encap(%s", colors.paren, colors.end);
    ofputil_format_port(a->tnl_port, fp->port_map, fp->s);
    ds_put_format(fp->s, "%s)%s", colors.paren, colors.end);
}

static enum ofperr
check_TUN_ENCAP(const struct ofpact_tun_cap *a OVS_UNUSED,
                struct ofpact_check_params *cp OVS_UNUSED)
{
    return 0;
}
...
static enum ofperr
decode_NXAST_RAW_TUN_DECAP(const struct nx_action_tun_cap *natd,
                           enum ofp_version ofp_version OVS_UNUSED,
                           struct ofpbuf *ofpacts)
{
    struct ofpact_tun_cap *tun_decap;

    tun_decap = ofpact_put_TUN_DECAP(ofpacts);
    tun_decap->tnl_port = natd->tnl_port;
    return 0;
}

static void
encode_TUN_DECAP(const struct ofpact_tun_cap *tun_decap,
                 enum ofp_version ofp_version OVS_UNUSED, struct ofpbuf *out)
{
    struct nx_action_tun_cap *natd = put_NXAST_TUN_DECAP(out);

    natd->tnl_port = tun_decap->tnl_port;
}

static char * OVS_WARN_UNUSED_RESULT
parse_TUN_DECAP(char *arg, const struct ofpact_parse_params *pp)
{
    struct ofpact_tun_cap *tun_decap;
    char *tnl_port_s = arg;

    tun_decap = ofpact_put_TUN_DECAP(pp->ofpacts);

    if (tnl_port_s && tnl_port_s[0]) {
        if (!ofputil_port_from_string(tnl_port_s, pp->port_map,
                                      &tun_decap->tnl_port)) {
            return xasprintf("%s: tun_decap tnl_port to unknown port",
                             tnl_port_s);
        }
    } else {
        return xasprintf("tun_decap must have tnl_port");
    }
...
static void
format_TUN_DECAP(const struct ofpact_tun_cap *a,
                 const struct ofpact_format_params *fp)
{
    ds_put_format(fp->s, "%stun_decap(%s", colors.paren, colors.end);
    ofputil_format_port(a->tnl_port, fp->port_map, fp->s);
    ds_put_format(fp->s, "%s)%s", colors.paren, colors.end);
}

static enum ofperr
check_TUN_DECAP(const struct ofpact_tun_cap *a OVS_UNUSED,
                struct ofpact_check_params *cp OVS_UNUSED)
{
    return 0;
}
