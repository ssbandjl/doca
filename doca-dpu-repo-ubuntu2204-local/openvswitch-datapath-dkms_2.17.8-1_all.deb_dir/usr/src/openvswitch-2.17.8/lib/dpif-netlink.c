...
static void
meter_police_ids_init_once(void)
{
    int n_ids = METER_POLICE_IDS_MAX - METER_POLICE_IDS_BASE + 1;

    if (!netdev_is_flow_api_enabled()) {
        return;
    }

    /* This will only be initialized (and policer actions range cleared)
     * in ovs-vswitchd process, and not in other processes that open
     * a netlink datapath like ovs-dpctl, so dpctl won't have
     * access to the global map. This is ok since ovs-dpctl
     * usage is deprecated.
     */

    meter_police_ids = id_pool_create(METER_POLICE_IDS_BASE, n_ids);
    tc_cleanup_policer_action(meter_police_ids,
