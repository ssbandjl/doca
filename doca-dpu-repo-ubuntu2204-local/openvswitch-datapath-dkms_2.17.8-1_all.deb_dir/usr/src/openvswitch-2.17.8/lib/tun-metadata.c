...
static const char *tun_type_names[] = {
    [TUN_TYPE_NONE] = "none",
    [TUN_TYPE_VXLAN] = "vxlan",
    [TUN_TYPE_GRE] = "gre",
    [TUN_TYPE_GENEVE] = "geneve",
};

const char *
tun_type_type2str(enum tun_type type)
{
    if (type >= ARRAY_SIZE(tun_type_names)) {
        return "invalid";
    }
    return tun_type_names[type];
}

int
tun_type_str2type(const char *str, enum tun_type *type)
{
    int i;

    for (i = 0; i < ARRAY_SIZE(tun_type_names); i++) {
        if (!strncmp(str, tun_type_names[i], strlen(tun_type_names[i]))) {
            *type = (enum tun_type) i;
            return 0;
        }
    }
    return -1;
