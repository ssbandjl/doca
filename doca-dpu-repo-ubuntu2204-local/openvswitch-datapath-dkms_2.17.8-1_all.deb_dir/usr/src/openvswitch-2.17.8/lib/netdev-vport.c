...
static int
derive_tunnel_config(const struct netdev *upper_netdev,
                     struct netdev *lower_netdev)
{
        const struct netdev_tunnel_config *upper_cfg;
        struct netdev_tunnel_config *lower_cfg;

        upper_cfg = netdev_get_tunnel_config(upper_netdev);
        lower_cfg = CONST_CAST(struct netdev_tunnel_config *,
                               netdev_get_tunnel_config(lower_netdev));
        if (upper_cfg == NULL || lower_cfg == NULL) {
            return EOPNOTSUPP;
        }

        lower_cfg->is_explicit = upper_cfg->is_explicit;
        return 0;
}
...
set_tunnel_config(struct netdev *dev_, const struct smap *args, char **errp)
{
    ...
    SMAP_FOR_EACH (node, args) {
        ...
        } else if (!strcmp(node->key, "explicit")) {
            tnl_cfg.is_explicit = !strcasecmp("true", node->value);
        } else {
...
get_tunnel_config(const struct netdev *dev, struct smap *args)
{
    ...
    if (tnl_cfg->is_explicit) {
        smap_add(args, "explicit", "true");
    }
...
#define TUNNEL_FUNCTIONS_COMMON                     \
    ...
    .derive_tunnel_config = derive_tunnel_config
        ...
        { "genev_sys",
          {
              ...
              .support_explicit_header = netdev_udp_tnl_support_explicit_header,
        ...
        { "gre_sys",
          {
              ...
              .support_explicit_header = netdev_gre_support_explicit_header,
              .get_ifindex = NETDEV_VPORT_GET_IFINDEX,
          },
          {{NULL, NULL, 0, 0}}
        },
