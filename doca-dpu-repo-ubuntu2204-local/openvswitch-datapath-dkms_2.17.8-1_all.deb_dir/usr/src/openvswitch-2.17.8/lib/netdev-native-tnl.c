                           ...
                           const struct ovs_action_push_tnl *data)
{
    ...
    /* check netdev provider entropy on original packet before pushing tunnel header */
    if (packet->orig_netdev) {
        err = netdev_packet_hw_entropy(packet->orig_netdev, packet, &entropy);
    }
...
bool
netdev_gre_support_explicit_header(const struct netdev *netdev,
                                   const struct dp_packet *packet)
{
    ...
    if (!(IP_VER(ip->ip_ihl_ver) == 4 && ip->ip_proto == IPPROTO_GRE) &&
        !(IP_VER(ip->ip_ihl_ver) == 6 && ip6->ip6_nxt == IPPROTO_GRE)) {
        return false;
    }

    tnl_cfg = netdev_get_tunnel_config(netdev);
    if (!tnl_cfg || !tnl_cfg->is_explicit) {
        return false;
    }
...
struct dp_packet *
netdev_gre_pop_header(struct dp_packet *packet, bool parsing)
{
    ...
    if (eth) {
        tnl->eth_src = eth->eth_src;
        tnl->eth_dst = eth->eth_dst;
    }
    if (parsing) {
        tnl->flags |= FLOW_TNL_F_EXPLICIT;
    }
    tnl->type = TUN_TYPE_GRE;
...
struct dp_packet *
netdev_erspan_pop_header(struct dp_packet *packet, bool parsing)
{
    const struct gre_base_hdr *greh;
...
struct dp_packet *
netdev_gtpu_pop_header(struct dp_packet *packet, bool parsing)
{
    struct pkt_metadata *md = &packet->md;
    struct flow_tnl *tnl = &md->tunnel;
    struct gtpuhdr *gtph;
...
bool
netdev_udp_tnl_support_explicit_header(const struct netdev *netdev,
                                       const struct dp_packet *packet)
{
    const struct netdev_tunnel_config *tnl_cfg;
    struct ovs_16aligned_ip6_hdr *ip6;
    ...
    if (!(IP_VER(ip->ip_ihl_ver) == 4 && ip->ip_proto == IPPROTO_UDP) &&
        !(IP_VER(ip->ip_ihl_ver) == 6 && ip6->ip6_nxt == IPPROTO_UDP)) {
        return false;
    }
    ...
    udp = dp_packet_l4(packet);
    if (!udp || udp->udp_dst != tnl_cfg->dst_port) {
        return false;
    }
...
struct dp_packet *
netdev_vxlan_pop_header(struct dp_packet *packet, bool parsing)
{
    struct pkt_metadata *md = &packet->md;
    struct flow_tnl *tnl = &md->tunnel;
    const struct eth_header *eth;
...
struct dp_packet *
netdev_geneve_pop_header(struct dp_packet *packet, bool parsing)
{
    struct pkt_metadata *md = &packet->md;
    struct flow_tnl *tnl = &md->tunnel;
