...
{
    set_eth_masked(dl_src, mask, &match->flow.tunnel.eth_src,
                   &match->wc.masks.tunnel.eth_src);
}
...
match_set_tun_eth_dst_masked(struct match *match,
                             ...
                             const struct eth_addr mask)
{
    set_eth_masked(dl_dst, mask, &match->flow.tunnel.eth_dst,
                   &match->wc.masks.tunnel.eth_dst);
}

void
match_set_tun_type(struct match *match, uint8_t type)
{
    match->wc.masks.tunnel.type = UINT8_MAX;
    match->flow.tunnel.type = type;
}

void
match_set_tun_type_masked(struct match *match, uint8_t type, uint8_t mask)
{
    match->wc.masks.tunnel.type = mask;
    match->flow.tunnel.type = type;
}
...
format_flow_tunnel(struct ds *s, const struct match *match)
{
    ...
    if (wc->masks.tunnel.type) {
        ds_put_format(s, "tun_type=%s,",
                      tun_type_type2str((enum tun_type) tnl->type));
    }
