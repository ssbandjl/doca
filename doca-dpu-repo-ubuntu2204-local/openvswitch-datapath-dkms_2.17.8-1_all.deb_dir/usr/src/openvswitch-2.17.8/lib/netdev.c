...
int
netdev_derive_tunnel_config(const struct netdev *upper_netdev,
                            struct netdev *lower_netdev)
{
    if (upper_netdev->netdev_class->derive_tunnel_config) {
        return upper_netdev->netdev_class->derive_tunnel_config(upper_netdev,
                                                                lower_netdev);
    }
    return EOPNOTSUPP;
}
...
bool
netdev_is_configured(const struct netdev *netdev)
{
    /* If no reconfigure callback is defined for this
     * netdev, it is already configured.
     */
    return netdev->reconfigure_status == 0 ||
           netdev->reconfigure_status == EOPNOTSUPP;
}
...
netdev_reconfigure(struct netdev *netdev)
{
    ...
    netdev->reconfigure_status = class->reconfigure
        ? class->reconfigure(netdev)
        : EOPNOTSUPP;
    return netdev->reconfigure_status;
}
