...
static void
netdev_dpdk_foreach_representor(dpdk_port_t esw_mgr_id,
                                void (*cb)(struct netdev_dpdk *, dpdk_port_t));
...
netdev_dpdk_doca_port_create(struct netdev *netdev)
{
    ...
    if (!ovs_doca_initialized()) {
        VLOG_ERR("%s: Failed to initialize DOCA port: DOCA is not initialized",
...
static void
netdev_dpdk_rep_set_esw_mgr_port_id(struct netdev_dpdk *dev,
                                    dpdk_port_t esw_mgr_port_id)
{
    ...
    OVS_REQUIRES(dev->mutex)
{
    ...
    if (netdev_dpdk_get_esw_mgr_port_id(netdev) == dev->port_id) {
        netdev_dpdk_foreach_representor(dev->port_id,
                                        netdev_dpdk_rep_set_esw_mgr_port_id);
    }
    return 0;
}
...
static void
netdev_dpdk_rep_stop(struct netdev_dpdk *dev,
                     dpdk_port_t esw_mgr_port_id OVS_UNUSED)
{
    if (ovs_doca_enabled()) {
        netdev_dpdk_doca_port_destroy(&dev->up);
    }
...
static void
netdev_dpdk_foreach_representor(dpdk_port_t esw_mgr_port_id,
                                void (*cb)(struct netdev_dpdk *, dpdk_port_t))
    OVS_REQUIRES(dpdk_mutex)
{
    struct rte_eth_dev_info info;
    ...
    rte_eth_dev_info_get(esw_mgr_port_id, &info);
    domain_id = info.switch_info.domain_id;
    LIST_FOR_EACH (dev, list_node, &dpdk_list) {
        rte_eth_dev_info_get(dev->port_id, &info);
        if (info.switch_info.domain_id != domain_id ||
...
netdev_dpdk_destruct(struct netdev *netdev)
{
    ...
    if (dev->started) {
        if (netdev_dpdk_get_esw_mgr_port_id(netdev) == dev->port_id) {
            netdev_dpdk_foreach_representor(dev->port_id, netdev_dpdk_rep_stop);
        }
        if (ovs_doca_enabled()) {
            netdev_dpdk_doca_port_destroy(netdev);
        }
        rte_eth_dev_stop(dev->port_id);
        dev->started = false;
    }
    ...
    if (dev->attached) {
        ...
        if (!dpdk_resources_still_used && netdev_is_configured(netdev)) {
...
static char *
netdev_dpdk_generate_devargs(const char *name, char *devargs, size_t maxlen)
{
     ...
     * the format of ../../.../<lower-dev>. Extract the lower device, and
     * return the result for it (recursive call).
     */
    if (snprintf(tmp, sizeof tmp, "/sys/class/net/%s/lower_p0", name) < 0) {
        return NULL;
    }
    len = readlink(tmp, device, sizeof device);
    if (len > 0) {
        ...
        if (slash) {
            str = netdev_dpdk_generate_devargs(slash + 1, devargs, maxlen);
        }
        return str;
    }
    ...
    /* In some kernels, there is a prefix of "c1". Ignore it. */
    if (!strncmp(phys_port_name, "c1", 2)) {
        phys_port_name += 2;
    }
    ...
    if (ovs_doca_enabled()) {
        mlx5_devargs = "dv_xmeta_en=4,dv_flow_en=2";
    } else {
                       ...
                       char **errp)
{
    ...
    iface_name = smap_get(args, "iface-name");
    new_devargs = smap_get(args, "dpdk-devargs");

    if (new_devargs == NULL) {
        new_devargs = netdev_dpdk_generate_devargs(netdev_get_name(netdev),
                                                   generated, sizeof generated);
        if (new_devargs) {
            smap_add(CONST_CAST(struct smap *, args), "dpdk-devargs",
                     new_devargs);
        }

        if (!iface_name) {
            /* When using generated devargs also use netdev as iface_name. */
            iface_name = netdev_get_name(netdev);
        }
    }
    ...
    if (iface_name && get_phys_port_name(iface_name, NULL, 0)) {
        VLOG_DBG("%s: Configured iface-name '%s' was not found in sysfs, "
                 "skipping DPDK probe", netdev_get_name(netdev), iface_name);
        err = ENODEV;
        goto out;
    }
...
netdev_dpdk_reconfigure(struct netdev *netdev)
{
    ...
    netdev->n_rxq = dev->requested_n_rxq;
    if (ovs_doca_enabled() && dev->requested_n_rxq != 1) {
        VLOG_WARN("%s: DOCA port supports only one RX queue. Force it.",
                  netdev_get_name(netdev));
        netdev->n_rxq = 1;
    }
...
netdev_dpdk_indirect_action_create(struct netdev *netdev,
