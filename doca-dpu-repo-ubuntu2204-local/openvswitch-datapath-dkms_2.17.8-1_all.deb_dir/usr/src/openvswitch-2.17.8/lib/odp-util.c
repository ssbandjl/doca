...
odp_action_len(uint16_t type)
{
    ...
    case OVS_ACTION_ATTR_TUN_DECAP: return sizeof(uint32_t);
...
format_odp_hash_action(struct ds *ds, const struct ovs_action_hash *hash_act)
{
    ...
    } else if (hash_act->hash_alg == OVS_HASH_ALG_DOCA) {
        ds_put_format(ds, "doca(%"PRIu32")", hash_act->hash_basis);
    } else {
...
static const struct attr_len_tbl ovs_tun_key_attr_lens[OVS_TUNNEL_KEY_ATTR_MAX + 1] = {
    ...
    [OVS_TUNNEL_KEY_ATTR_ETH_SRC]        = { .len = ETH_ADDR_LEN },
    [OVS_TUNNEL_KEY_ATTR_ETH_DST]        = { .len = ETH_ADDR_LEN },
    [OVS_TUNNEL_KEY_ATTR_EXPLICIT]       = { .len = 0 },
    [OVS_TUNNEL_KEY_ATTR_TYPE]           = { .len = 1 },
};
                        ...
                        struct flow_tnl *tun, char **errorp)
{
    ...
    NL_NESTED_FOR_EACH(a, left, attr) {
        ...
        case OVS_TUNNEL_KEY_ATTR_ETH_DST:
            ...
            break;
        case OVS_TUNNEL_KEY_ATTR_EXPLICIT:
            tun->flags |= FLOW_TNL_F_EXPLICIT;
            break;

        case OVS_TUNNEL_KEY_ATTR_TYPE:
            tun->type = nl_attr_get_u8(a);
            break;
                ...
                const struct ofpbuf *key_buf, const char *tnl_type)
{
    ...
    if (tun_key->flags & FLOW_TNL_F_EXPLICIT) {
        nl_msg_put_flag(a, OVS_TUNNEL_KEY_ATTR_EXPLICIT);
    }
    ...
    if (!eth_addr_is_zero(tun_key->eth_src)) {
        ...
        eth = nl_msg_put_unspec_uninit(a, OVS_TUNNEL_KEY_ATTR_ETH_SRC,
                                       sizeof *eth);
        *eth = tun_key->eth_src;
    }
    if (!eth_addr_is_zero(tun_key->eth_dst)) {
        ...
        eth = nl_msg_put_unspec_uninit(a, OVS_TUNNEL_KEY_ATTR_ETH_DST,
                                       sizeof *eth);
        *eth = tun_key->eth_dst;
    }
    if (tun_key->type != TUN_TYPE_NONE) {
        nl_msg_put_u8(a, OVS_TUNNEL_KEY_ATTR_TYPE, tun_key->type);
    }
                    ...
                    struct ds *ds, bool verbose)
{
    ...
    NL_NESTED_FOR_EACH(a, left, attr) {
        ...
        case OVS_TUNNEL_KEY_ATTR_GTPU_OPTS:
            ...
            break;
        case OVS_TUNNEL_KEY_ATTR_ETH_SRC:
            format_eth(ds, "eth_src", nl_attr_get_eth_addr(a),
                       ma ? nl_attr_get(ma) : NULL, verbose);
            break;
        case OVS_TUNNEL_KEY_ATTR_ETH_DST:
            format_eth(ds, "eth_dst", nl_attr_get_eth_addr(a),
                       ma ? nl_attr_get(ma) : NULL, verbose);
            break;
        case OVS_TUNNEL_KEY_ATTR_EXPLICIT:
            flags |= FLOW_TNL_F_EXPLICIT;
            break;
        case OVS_TUNNEL_KEY_ATTR_TYPE: {
            const uint8_t *mask = ma ? nl_attr_get(ma) : NULL;
            uint8_t key = nl_attr_get_u8(a);
            bool mask_full;

            mask_full = !mask || *mask == UINT8_MAX;
            if (verbose || mask_full) {
                ds_put_format(ds, "type=%s%s",
                              tun_type_type2str((enum tun_type) key),
                              mask_full ? "" : "/none");
                ds_put_char(ds, ',');
            }
            break;
    ...
    if (mask_attr) {
        NL_NESTED_FOR_EACH(a, left, mask_attr) {
            ...
            case OVS_TUNNEL_KEY_ATTR_OAM:
                ...
                break;
            case OVS_TUNNEL_KEY_ATTR_EXPLICIT:
                mask_flags |= FLOW_TNL_F_EXPLICIT;
                break;
            }
        }
    }
