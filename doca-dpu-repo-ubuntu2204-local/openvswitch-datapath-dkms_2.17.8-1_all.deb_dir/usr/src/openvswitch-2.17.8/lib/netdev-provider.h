...
/* ## ---------------- ## */
    ...
    /* Derive tunnel configuration from a upper layer (dpif) netdev, to a
     * lower layer (datapath) netdev.
     */
    int (*derive_tunnel_config)(const struct netdev *upper_netdev,
     ...
     * for further processing. The parsing flag indicates the call is for
     * parsing the outer header to tunnel metadata only.
     * Returns NULL in case of error or tunnel implementation queued packet for
     * further processing. */
    struct dp_packet * (*pop_header)(struct dp_packet *packet, bool parsing);

    /* Checks if the packet is supported to be processed on an explicit tunnel
     * netdev.
     */
    bool (*support_explicit_header)(const struct netdev *,
