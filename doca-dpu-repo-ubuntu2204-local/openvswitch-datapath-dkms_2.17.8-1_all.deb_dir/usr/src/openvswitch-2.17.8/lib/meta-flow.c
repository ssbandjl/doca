             ...
             const union mf_value *value, struct match *match, char **err_str)
{
    ...
    case MFF_TUN_ETH_DST:
        ...
        break;
    case MFF_TUN_TYPE:
        match_set_tun_type(match, value->u8);
        break;
    CASE_MFF_TUN_METADATA:
...
mf_set_wild(const struct mf_field *mf, struct match *match, char **err_str)
{
    ...
    case MFF_TUN_ETH_DST:
        ...
        break;
    case MFF_TUN_TYPE:
        match->flow.tunnel.type = 0;
        match->wc.masks.tunnel.type = 0;
        break;
    CASE_MFF_TUN_METADATA:
       ...
       struct match *match, char **err_str)
{
    ...
    case MFF_TUN_ETH_DST:
        ...
        break;
    case MFF_TUN_TYPE:
        match_set_tun_type_masked(match, value->u8, mask->u8);
        break;
    CASE_MFF_TUN_METADATA:
...
static char *
mf_from_tnl_type_string(const char *s, uint8_t *valuep, uint8_t *maskp)
{
    enum tun_type type;

    if (tun_type_str2type(s, &type) < 0) {
        return xasprintf("%s: unknown tun_type", s);
    }
         ...
         union mf_value *value, union mf_value *mask)
{
    ...
    case MFS_TNL_TYPE:
        error = mf_from_tnl_type_string(s, &value->u8, &mask->u8);
        break;
...
static void
mf_format_tnl_type_string(uint8_t value, uint8_t mask, struct ds *s)
{
    ...
    ds_put_cstr(s, tun_type_type2str((enum tun_type) value));
}
          ...
          struct ds *s)
{
    ...
    case MFS_TNL_TYPE:
        mf_format_tnl_type_string(value->u8, mask ? mask->u8 : UINT8_MAX, s);
        break;
