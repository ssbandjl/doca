...
def ovs_checkpatch_parse(text, filename, author=None, committer=None):
    ...
    is_gerrit_change_id = re.compile(r'(\s*(change-id: )(.*))$',
                                     re.I | re.M | re.S)
    cherry_picked_commit = re.compile(r'^\(cherry picked from commit .*',
                                      re.I | re.M | re.S)
            ...
            if seppatch.match(line):
                ...
                if not skip_signoff_check:
                    ...
                    extra_sigs = [x for x in signatures
                                  ...
                                  and x != committer]
                    if (not_cherry_pick
                        and (len(extra_sigs) > 1 or (committer and extra_sigs))):
                ...
                print("%d: %s\n" % (lineno, line))
            elif cherry_picked_commit.match(line):
                not_cherry_pick = False
                continue
            elif spellcheck:
