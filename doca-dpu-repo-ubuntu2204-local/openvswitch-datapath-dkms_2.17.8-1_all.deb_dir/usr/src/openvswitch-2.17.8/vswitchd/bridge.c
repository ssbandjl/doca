    ...
    HMAP_FOR_EACH (br, node, &all_bridges) {
        ...
        HMAP_FOR_EACH (port, hmap_node, &br->ports) {
            ...
            LIST_FOR_EACH (iface, port_elem, &port->ifaces) {
                if (!netdev_is_configured(iface->netdev)) {
                    continue;
                }
...
bridge_run(void)
{
    ...
    if (daemonize_txn) {
        ...
        if (status != TXN_INCOMPLETE) {
            ...
            VLOG_INFO_ONCE("%s (Open vSwitch) %s%s, dpdk:%s, doca:%s",
                           program_name, VERSION, ovs_git_sha_str,
                           dpdk_get_version(), ovs_doca_get_version());
        }
    }
...
iface_lookup(const struct bridge *br, const char *name)
{
    ...
    HMAP_FOR_EACH_WITH_HASH (iface, name_node, hash_string(name, 0),
                             &br->iface_by_name) {
        if (!strcmp(iface->name, name) && netdev_is_configured(iface->netdev)) {
            return iface;
        }
    }
