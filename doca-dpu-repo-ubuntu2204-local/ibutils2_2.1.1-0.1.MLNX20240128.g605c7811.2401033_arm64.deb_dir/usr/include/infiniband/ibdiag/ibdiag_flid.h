...
private:
    ...
    int DumpAdjSubnets(ostream &outStream);
    int DumpEnabledFLIDsOnRouters(ostream &outStream);
    void LocalEnabledFLIDsToStream(const IBNode &router,
                                   const SMP_RouterInfo &routerInfo, ostream &outStream);
    void NonLocalEnabledFLIDsToStream(const IBNode &router, ostream &outStream);
    int DumpFLIDsPerSwicthes(ostream &outStream);
    ...
    void CollectEnabledFLIDs(u_int32_t from, u_int32_t to, IBNode &router) const;
    void FindCommonLids();
    ...
    lid_to_nodes_map       swicthesByFLID;
    node_to_lids_map       swicthesWithMultipleFLIDs;
    vec_lids               common;
