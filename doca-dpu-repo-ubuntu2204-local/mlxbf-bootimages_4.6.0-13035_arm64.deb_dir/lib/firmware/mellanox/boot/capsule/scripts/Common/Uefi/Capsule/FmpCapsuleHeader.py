...
class FmpCapsuleImageHeaderClass (object):
    ...
    def __init__ (self, Version = EFI_FIRMWARE_MANAGEMENT_CAPSULE_IMAGE_HEADER_INIT_VERSION):
        if Version > EFI_FIRMWARE_MANAGEMENT_CAPSULE_IMAGE_HEADER_INIT_VERSION:
            raise ValueError
        self._Valid                 = False
        ...
        # Select the header format and size based on the version.
        if Version < EFI_FIRMWARE_MANAGEMENT_CAPSULE_IMAGE_HEADER_INIT_VERSION:
            self._StructFormat = '<I16sB3BIIQ'
        else:
            self._StructFormat = '<I16sB3BIIQQ'
        self._StructSize = struct.calcsize (self._StructFormat)

    def Encode (self):
        self.UpdateImageSize      = len (self.Payload)
        self.UpdateVendorCodeSize = len (self.VendorCodeBytes)
        if self.Version < EFI_FIRMWARE_MANAGEMENT_CAPSULE_IMAGE_HEADER_INIT_VERSION:
    ...
    def Decode (self, Buffer):
        ...
        self.ImageCapsuleSupport    = 0x0000000000000000 if Version < EFI_FIRMWARE_MANAGEMENT_CAPSULE_IMAGE_HEADER_INIT_VERSION else ImageCapsuleSupport
...
class FmpCapsuleHeaderClass (object):
    ...
    def AddPayload (self, UpdateImageTypeId, Payload = b'', VendorCodeBytes = b'', HardwareInstance = 0, UpdateImageIndex = 1, CapsuleSupport = 0, ImageHeaderVersion = EFI_FIRMWARE_MANAGEMENT_CAPSULE_IMAGE_HEADER_INIT_VERSION):
        self._PayloadList.append ((UpdateImageTypeId, Payload, VendorCodeBytes, HardwareInstance, UpdateImageIndex, CapsuleSupport, ImageHeaderVersion))
    ...
    def Encode (self, ImageHeaderVersion = EFI_FIRMWARE_MANAGEMENT_CAPSULE_IMAGE_HEADER_INIT_VERSION):
        ...
        Index = 1
        for (UpdateImageTypeId, Payload, VendorCodeBytes, HardwareInstance, UpdateImageIndex, CapsuleSupport, ImageHeaderVersion) in self._PayloadList:
            FmpCapsuleImageHeader = FmpCapsuleImageHeaderClass (Version = ImageHeaderVersion)
            FmpCapsuleImageHeader.Version                = ImageHeaderVersion
    ...
    def Decode (self, Buffer, ImageHeaderVersion = EFI_FIRMWARE_MANAGEMENT_CAPSULE_IMAGE_HEADER_INIT_VERSION):
        if len (Buffer) < self._StructSize:
            raise ValueError
