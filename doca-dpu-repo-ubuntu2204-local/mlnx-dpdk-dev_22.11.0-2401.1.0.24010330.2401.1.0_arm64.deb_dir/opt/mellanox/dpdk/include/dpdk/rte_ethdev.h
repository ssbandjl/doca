 ...
 *
 * If the port is used as a host port for sharing flow engine resources
 * with other devices (configurable through rte_flow_configure()),
 * then this port might not be closed before guest ports are closed.
 * PMDs which support sharing flow engine resources might return
 * (-EBUSY) in that case to notify about port dependencies.
 *
 * If (-EBUSY) is returned, port resources are not released.
 *
 * @param port_id
 *   The port identifier of the Ethernet device.
 * @return
 ...
 *     - (-EBUSY) if some other port depends on resources
 *       allocated by the port being closed.
 */
