...
/**
 * Specialize table for actions only, a pool for other tables.
 */
#define RTE_FLOW_TABLE_SPECIALIZE_RESOUCE_POOL RTE_BIT32(3)
/**
 * Specialize table for using actions from other pool.
 * this table has no actions resources from itself.
 */
#define RTE_FLOW_TABLE_SPECIALIZE_RESOUCE_USE_POOL RTE_BIT32(4)
 ...
 * @b EXPERIMENTAL: this API may change without prior notice.
 *
 * Template table rules insertion type.
 */
 ...
 * @b EXPERIMENTAL: this API may change without prior notice.
 *
 * Template table hash calculation function.
 */
...
#ifdef RTE_FLOW_ASYNC_DEBUG
#include <rte_errno.h>

extern int rte_eth_dev_is_valid_port(uint16_t port_id);
#endif
 ...
 * @b EXPERIMENTAL: this API may change without prior notice.
 *
 * Destination for the entropy calculation.
 *
 * @see function rte_flow_calc_encap_entropy
 */
enum rte_flow_entropy_dest {
	/* The entropy will be added to src port field. */
	RTE_FLOW_ENTROPY_DEST_UDP_SRC_PORT,
	/* The entropy will be added to GRE flow id field. */
	RTE_FLOW_ENTROPY_DEST_GRE_FLOW_ID
};
...
typedef int (*rte_flow_async_update_t)(struct rte_eth_dev *dev,
				       uint32_t queue_id,
...
/** @internal Enqueue flow rule update operation following the template table resize. */
typedef int (*rte_flow_async_update_resized_t)(struct rte_eth_dev *dev,
					       uint32_t queue,
					       const struct rte_flow_op_attr *attr,
...
/** @internal Calculate the encap entropy to be set in the dest. */
typedef int (*rte_flow_calc_encap_entropy_t)(struct rte_eth_dev *dev,
					     const struct rte_flow_item pattern[],
					     enum rte_flow_entropy_dest dest_field,
					     uint8_t *entropy,
					     struct rte_flow_error *error);
...
struct rte_flow_fp_ops {
	...
	rte_flow_async_update_t async_update;
	...
	rte_flow_async_update_resized_t flow_update_resized;
	rte_flow_calc_encap_entropy_t calc_encap_entropy;
} __rte_cache_aligned;
		      ...
		      struct rte_flow_error *error)
{
	...
	dev = ops->dev;
	return ops->async_create(dev, queue_id,
 ...
 * @b EXPERIMENTAL: this API may change without prior notice.
 *
 * Enqueue rule creation by index operation.
		      ...
		      struct rte_flow_error *error)
{
	...
	ops = &rte_flow_fp_ops[port_id];
	dev = ops->dev;
	return ops->async_create_by_index(dev, queue_id,
					  op_attr, template_table,
					  rule_index,
					  actions, actions_template_index,
					  user_data, error);
		       ...
		       struct rte_flow_error *error)
{
	...
	ops = &rte_flow_fp_ops[port_id];
	dev = ops->dev;
	return ops->async_destroy(dev, queue_id,
				  op_attr, flow,
				  user_data, error);
}
		      ...
		      struct rte_flow_error *error)
{
	...
	ops = &rte_flow_fp_ops[port_id];
	dev = ops->dev;
	return ops->async_update(dev, queue_id,
				 op_attr, flow,
				 actions, actions_template_index,
				 user_data, error);
}
	      ...
	      struct rte_flow_error *error)
{
	...
	ops = &rte_flow_fp_ops[port_id];
	dev = ops->dev;
	return ops->push(dev, queue_id, error);
}
	      ...
	      struct rte_flow_error *error)
{
	...
	ops = &rte_flow_fp_ops[port_id];
	dev = ops->dev;
	return ops->pull(dev, queue_id, res, n_res, error);
}
		...
		struct rte_flow_error *error)
{
	...
	ops = &rte_flow_fp_ops[port_id];
	dev = ops->dev;
	return ops->async_action_handle_create(dev, queue_id,
		...
		struct rte_flow_error *error)
{
	...
	ops = &rte_flow_fp_ops[port_id];
	dev = ops->dev;
	return ops->async_action_handle_destroy(dev, queue_id, op_attr, action_handle,
						user_data, error);
}
		...
		struct rte_flow_error *error)
{
	...
	ops = &rte_flow_fp_ops[port_id];
	dev = ops->dev;
	return ops->async_action_handle_update(dev, queue_id, op_attr, action_handle, update,
					       user_data, error);
}
		...
		struct rte_flow_error *error)
{
	...
	ops = &rte_flow_fp_ops[port_id];
	dev = ops->dev;
	return ops->async_action_handle_query(dev, queue_id, op_attr, action_handle, data,
					      user_data, error);
}
					  ...
					  struct rte_flow_error *error)
{
	...
	ops = &rte_flow_fp_ops[port_id];
	dev = ops->dev;
	return ops->async_action_handle_query_update(dev, queue_id, attr, handle,
						     update, query, mode,
						     user_data, error);
}
					 ...
					 struct rte_flow_error *error)
{
	...
	ops = &rte_flow_fp_ops[port_id];
	dev = ops->dev;
	return ops->async_action_list_handle_create(dev, queue_id,
						    attr, conf, actions,
						    user_data, error);
}
		 ...
		 void *user_data, struct rte_flow_error *error)
{
	...
	ops = &rte_flow_fp_ops[port_id];
	dev = ops->dev;
	return ops->async_action_list_handle_destroy(dev, queue_id, op_attr, handle,
						     user_data, error);
}
					       ...
					       struct rte_flow_error *error)
{
	...
	ops = &rte_flow_fp_ops[port_id];
	dev = ops->dev;
	return ops->async_action_list_handle_query_update(dev, queue_id, attr, handle,
							  update, query, mode,
							  user_data, error);
}
			      ...
			      struct rte_flow_error *error)
{
	...
	ops = &rte_flow_fp_ops[port_id];
	dev = ops->dev;
	return ops->flow_update_resized(dev, queue, attr, rule,
					user_data, error);
}
 ...
 * @b EXPERIMENTAL: this API may change without prior notice.
 *
 * Calculate the entropy for a given pattern.
 *
 * @param port_id
 *   Port identifier of Ethernet device.
 * @param pattern
 ...
 * @param dest_field
 *   The field to which the HW will write the entropy.
 * @param entropy
 *   Used to return the calculated entropy, in network order byte[0] is the msb.
 *   The number of bytes is based on the dest target.
 * @param error
 *   Perform verbose error reporting if not NULL.
 *   PMDs initialize this structure in case of error only.
 *
 ...
 *   - (-ENODEV) if *port_id* invalid.
 *   - (-ENOTSUP) if underlying device does not support this functionality.
 *   - (-EINVAL) if *pattern* doesn't hold information to calculate the entropy
			    ...
			    struct rte_flow_error *error)
{
	...
	ops = &rte_flow_fp_ops[port_id];
	dev = ops->dev;
	return ops->calc_encap_entropy(dev, pattern, dest_field, entropy, error);
}
