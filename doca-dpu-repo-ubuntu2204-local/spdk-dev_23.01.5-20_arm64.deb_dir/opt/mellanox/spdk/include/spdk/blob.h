...
/**
 * Determine if the blob is healthy, ready to perform IO.
 *
 * \param blob A blob
 *
 * \return true if the blob and any snapshots upon which it depends are healthy, else false.
 */
bool spdk_blob_is_healthy(const struct spdk_blob *blob);
