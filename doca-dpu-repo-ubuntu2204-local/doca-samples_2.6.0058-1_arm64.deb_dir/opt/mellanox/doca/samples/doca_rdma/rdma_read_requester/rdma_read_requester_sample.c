...
write_read_connection(struct rdma_config *cfg, struct rdma_resources *resources)
{
	...
	if (result != DOCA_SUCCESS) {
		DOCA_LOG_ERR("Failed to read the remote RDMA mmap connection details: %s", doca_error_get_descr(result));
		return result;
	}
 ...
 */
static void
rdma_read_completed_callback(struct doca_rdma_task_read *rdma_read_task,
 ...
 */
static void
rdma_read_error_callback(struct doca_rdma_task_read *rdma_read_task,
			 union doca_data task_user_data, union doca_data ctx_user_data)
{
	struct rdma_resources *resources = (struct rdma_resources *)ctx_user_data.ptr;
 ...
 */
static doca_error_t
rdma_read_requester_export_and_connect(struct rdma_resources *resources)
{
	doca_error_t result;
 ...
 */
static doca_error_t
rdma_read_prepare_and_submit_task(struct rdma_resources *resources)
{
	...
	result = doca_buf_inventory_buf_get_by_data(resources->buf_inventory, resources->remote_mmap, remote_mmap_range,
	...
	result = doca_rdma_task_read_allocate_init(resources->rdma, resources->src_buf, resources->dst_buf,
						   task_user_data, &rdma_read_task);
	if (result != DOCA_SUCCESS) {
 ...
 */
static void
rdma_read_requester_state_change_callback(const union doca_data user_data, struct doca_ctx *ctx,
					  enum doca_ctx_states prev_state, enum doca_ctx_states next_state)
{
	...
	case DOCA_CTX_STATE_STARTING:
		...
		if (result != DOCA_SUCCESS)
			DOCA_LOG_ERR("rdma_read_requester_export_and_connect() failed: %s", doca_error_get_descr(result));
		else
...
rdma_read_requester(struct rdma_config *cfg)
{
	...
	result = allocate_rdma_resources(cfg, mmap_permissions, rdma_permissions,
					 doca_rdma_cap_task_read_is_supported, &resources);
	if (result != DOCA_SUCCESS) {
	...
	/*
	 * Run the progress engine which will run the state machine defined in rdma_read_requester_state_change_callback()
