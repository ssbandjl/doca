 ...
 */
static doca_error_t
create_udp_pipe(struct doca_flow_port *port, struct doca_flow_pipe *fwd_pipe,
		struct entries_status *status, struct doca_flow_pipe **pipe)
{
	struct doca_flow_match match;
	struct doca_flow_pipe_cfg cfg;
	struct doca_flow_fwd fwd;
 ...
 */
static doca_error_t
create_ct_pipe(struct doca_flow_port *port, struct doca_flow_pipe *fwd_pipe, struct doca_flow_pipe *fwd_miss_pipe, struct doca_flow_pipe **pipe)
{
	struct doca_flow_match match;
 ...
 */
static doca_error_t
create_vxlan_encap_pipe(struct doca_flow_port *port, int port_id, struct entries_status *status, struct doca_flow_pipe **pipe)
{
	struct doca_flow_match match;
...
process_packets(struct doca_flow_port *port, uint16_t ct_queue, struct entries_status *ct_status)
{
	...
	for (i = 0; i < PACKET_BURST && i < nb_packets; i++) {
		...
		result = doca_flow_ct_add_entry(ct_queue, NULL, flags, &match_o, &match_r, 0, 0, 0, ct_status, &entry);
		if (result != DOCA_SUCCESS) {
 ...
 */
doca_error_t
flow_ct_udp(uint16_t nb_queues, struct doca_dev *ct_dev)
{
	...
	uint32_t ct_flags, nb_arm_queues = 1, nb_ctrl_queues = 1, nb_user_actions = 0, nb_ipv4_sessions = 1024, nb_ipv6_sessions = 0; /* On BF2 should always be 0 */
