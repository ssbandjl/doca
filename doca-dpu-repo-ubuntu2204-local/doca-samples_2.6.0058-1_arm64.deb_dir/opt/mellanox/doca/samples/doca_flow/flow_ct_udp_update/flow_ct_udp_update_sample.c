 ...
 */
static doca_error_t
process_packets(struct doca_flow_port *port, uint16_t ct_queue, struct entries_status *ct_status, struct doca_flow_pipe_entry *ct_entries_arr[PACKET_BURST], int *ct_entries_arr_len)
{
	...
	for (i = 0; i < PACKET_BURST && i < nb_packets; i++) {
		...
		result = doca_flow_ct_add_entry(ct_queue, NULL, flags, &match_o, &match_r, 0, 0, 0, ct_status, &ct_entries_arr[i]);
		if (result != DOCA_SUCCESS) {
 ...
 */
static doca_error_t
update_nonactive_entries(struct doca_flow_port *port, struct doca_flow_pipe_entry **entries_arr, int entries_arr_len, uint16_t ct_queue)
{
	struct doca_flow_query query_o, query_r;
	...
	for (i = 0; i < entries_arr_len; i++) {
		result = doca_flow_ct_query_entry(ct_queue, NULL, flags, entries_arr[i],
						  &query_o, &query_r, &last_hit_s);
		if (result != DOCA_SUCCESS) {
		...
		if (query_o.total_pkts == 0 && query_r.total_pkts == 0) {
			result = doca_flow_ct_update_entry(ct_queue, NULL, flags, entries_arr[i], 0, 0, aging_s);
			if (result != DOCA_SUCCESS) {
				DOCA_LOG_ERR("Failed to query CT entry: %s", doca_error_get_descr(result));
				return result;
			}
 ...
 */
doca_error_t
flow_ct_udp_update(uint16_t nb_queues, struct doca_dev *ct_dev)
{
	...
	result = update_nonactive_entries(ports[0], ct_entries_arr, ct_entries_arr_len, ct_queue);
	if (result != DOCA_SUCCESS)
		goto cleanup;
