	       ...
	       struct doca_flow_pipe **pipe)
{
	...
	struct doca_flow_actions *actions_mask_list[] = {&meta_action, &nat_action};
	doca_error_t result;
	...
	cfg.match_mask = &mask;
	cfg.port = port;
	cfg.attr.nb_actions = 2;
	cfg.actions = actions_list;
	cfg.actions_masks = actions_mask_list;
	cfg.action_descs = NULL;
	...
	meta_action.meta.pkt_meta = 0xffff;
	meta_action.meta.u32[0] = 0xffff;

	/* NAT action */
	nat_action.action_idx = 1;
	nat_action.outer.l3_type = DOCA_FLOW_L3_TYPE_IP4;
	nat_action.outer.ip4.src_ip = 0xffffffff;
	nat_action.outer.ip4.dst_ip = 0xffffffff;
	nat_action.outer.l4_type_ext = DOCA_FLOW_L4_TYPE_EXT_TCP;
	nat_action.outer.transport.src_port = 0xffff;
	nat_action.outer.transport.dst_port = 0xffff;
...
/*
 * Create TCP pipe with 2 entries, one for each port
 *
 * @port [in]: Pipe port
...
process_packets(struct doca_flow_port *port, uint16_t ct_queue, struct entries_status *ct_status,
		uint32_t shared_handle, struct doca_flow_pipe_entry **entry)
{
	...
	for (i = 0; i < PACKET_BURST && i < nb_packets; i++) {
		...
		if (tcp_state & DOCA_FLOW_MATCH_TCP_FLAG_SYN) {
			...
			nb_process = ct_status->nb_processed;
			result = doca_flow_ct_add_entry_with_actions(ct_queue, NULL, flags, &match_o, &match_r,
								     &shared_action_o, &action_r, 0, ct_status, entry);
			if (result != DOCA_SUCCESS) {
				DOCA_LOG_ERR("Failed to add CT pipe an entry: %s", doca_error_get_descr(result));
				return result;
			}

			while (ct_status->nb_processed != nb_process + 1) {
		...
		rte_eth_tx_burst(1, 0, &packets[i], 1);
	}
 ...
 */
doca_error_t
flow_ct_tcp_actions(uint16_t nb_queues, struct doca_dev *ct_dev)
{
	...
	ct_flags = DOCA_FLOW_CT_FLAG_NO_AGING | DOCA_FLOW_CT_FLAG_USER_ACTIONS;
	...
	shared_action.data.meta.pkt_meta = 1;
	shared_action.data.meta.u32[0] = 1;
