...
flow_ct_udp_query(uint16_t nb_queues, struct doca_dev *ct_dev)
{
	...
	result = doca_flow_ct_query_entry(ct_queue, NULL, DOCA_FLOW_CT_ENTRY_FLAGS_NO_WAIT, ct_entry, &query_o,
					  &query_r, &last_hit_s);
	if (result != DOCA_SUCCESS) {
	...
	DOCA_LOG_INFO("Origin Total bytes: %ld", query_o.total_bytes);
	DOCA_LOG_INFO("Origin Total packets: %ld", query_o.total_pkts);
	DOCA_LOG_INFO("Reply Total bytes: %ld", query_r.total_bytes);
	DOCA_LOG_INFO("Reply Total packets: %ld", query_r.total_pkts);
