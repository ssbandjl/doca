...
DOCA_LOG_REGISTER(CC_DATA_PATH_HIGH_SPEED_CLIENT::MAIN);

/* Sample's Logic */
doca_error_t start_cc_data_path_client_sample(const char *server_name, const char *dev_pci_addr, const char *text);
...
main(int argc, char **argv)
{
	...
	result = doca_argp_init("doca_cc_data_path_client", &cfg);
	if (result != DOCA_SUCCESS) {
	...
	/* Start the client */
	result = start_cc_data_path_client_sample(server_name, cfg.cc_dev_pci_addr, cfg.text);
	if (result != DOCA_SUCCESS) {
