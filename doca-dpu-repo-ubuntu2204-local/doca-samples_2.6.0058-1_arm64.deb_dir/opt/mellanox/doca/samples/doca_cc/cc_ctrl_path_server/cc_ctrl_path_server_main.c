...
DOCA_LOG_REGISTER(CC_CTRL_PATH_SERVER::MAIN);

/* Sample's Logic */
doca_error_t start_cc_ctrl_path_server_sample(const char *server_name, const char *dev_pci_addr,
...
main(int argc, char **argv)
{
	...
	result = doca_argp_init("doca_cc_ctrl_path_server", &cfg);
	if (result != DOCA_SUCCESS) {
	...
	/* Start the server */
	result = start_cc_ctrl_path_server_sample(server_name, cfg.cc_dev_pci_addr, cfg.cc_dev_rep_pci_addr, cfg.text);
	if (result != DOCA_SUCCESS) {
