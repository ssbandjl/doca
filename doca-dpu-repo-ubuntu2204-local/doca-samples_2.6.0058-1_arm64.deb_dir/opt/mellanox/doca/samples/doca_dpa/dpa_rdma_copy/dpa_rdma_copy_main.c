...
DOCA_LOG_REGISTER(DPA_RDMA_COPY::MAIN);

/* Sample's Logic */
doca_error_t dpa_rdma_copy(struct dpa_resources *resources);
...
main(int argc, char **argv)
{
	...
	result = doca_argp_init("doca_dpa_rdma_copy", &cfg);
	if (result != DOCA_SUCCESS) {
	...
	/* Running sample */
	result = dpa_rdma_copy(&resources);
	if (result != DOCA_SUCCESS) {
		DOCA_LOG_ERR("dpa_rdma_copy() encountered an error: %s", doca_error_get_descr(result));
		goto dpa_cleanup;
	}
