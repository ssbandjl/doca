...
{
	return doca_sync_event_cap_is_export_to_dpu_supported(devinfo);
}
...
se_init(struct sync_event_runtime_objects *se_rt_objs)
{
	...
	result = doca_sync_event_add_publisher_location_dpu(se_rt_objs->se);
	if (result != DOCA_SUCCESS) {
...
cc_init(struct sync_event_runtime_objects *se_rt_objs)
{
	...
	if (result != DOCA_SUCCESS) {
		DOCA_LOG_ERR("Failed to connect DPU: %s", doca_error_get_descr(result));
		return result;
	}
...
se_export(struct sync_event_runtime_objects *se_rt_objs)
{
	...
	if (result != DOCA_SUCCESS) {
		DOCA_LOG_ERR("Failed to export sync event to DPU: %s", doca_error_get_descr(result));
		return result;
	}
...
se_communicate_async(const struct sync_event_config *se_cfg, struct sync_event_runtime_objects *se_rt_objs)
{
	...
	if (se_cfg->is_update_atomic) {
		...
		result = doca_sync_event_task_notify_set_alloc_init(se_rt_objs->se, 1, user_data, &notify_set_task);
		if (result != DOCA_SUCCESS)
			return result;
	...
	result = doca_sync_event_task_wait_gt_alloc_init(se_rt_objs->se, 1, UINT64_MAX, user_data, &wait_gt_task);
	if (result != DOCA_SUCCESS)
		return result;
