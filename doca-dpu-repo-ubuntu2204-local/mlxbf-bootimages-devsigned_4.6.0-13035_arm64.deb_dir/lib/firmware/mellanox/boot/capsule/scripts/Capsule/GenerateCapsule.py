...
def SignPayloadOpenSsl (Payload, ToolPath, SignerPrivateKey, SignerPublicCertFile, OtherPublicCertFile, Verbose = False, Engine = None, MsgDigest = None):
    #
    ...
    if MsgDigest is None:
        MsgDigest = "sha256"
    Command = Command + 'smime -sign -binary -outform DER -md {DigestAlgo} '.format (DigestAlgo = MsgDigest)
    if OtherPublicCertFile is not None:
        Command = Command + '-certfile "{OtherCert}" '.format (OtherCert = OtherPublicCertFile)
    Command = Command + '-signer "{SignerCert}" '.format (SignerCert = SignerPublicCertFile)
    if Engine is not None:
        Command = Command + '-engine {OpenSslEngine} -inkey "{SignerKey}" '.format (
            OpenSslEngine = Engine, SignerKey = SignerPrivateKey)
        #
        # When an engine is specified, keys are assumed in an engine form.
        # This does not work for AWS CloudHSM that uses a safety key design
        # where a "fake" key is stored in a file outside the HSM, and this
        # key is a reference to the real key in the HSM.
        #
        if Engine == "cloudhsm":
            Command = Command + '-keyform PEM'
        else:
            Command = Command + '-keyform ENGINE'
    elif SignerPrivateKey[0:7] == "pkcs11:":
        Command = Command + '-engine pkcs11 -keyform engine -inkey "{SignerKey}"'.format (SignerKey = SignerPrivateKey)
    else:
        Command = Command + '-inkey "{SignerKey}"'.format (SignerKey = SignerPrivateKey)
    if Verbose:
        print (Command)
...
def VerifyPayloadOpenSsl (Payload, CertData, ToolPath, OtherPublicCertFile, Verbose = False):
    #
    # Create a temporary directory
    #
    TempDirectoryName = tempfile.mkdtemp()
    ...
    Command = Command + '-content {Content} -CAfile "{Public}"'.format (Content = TempFileName, Public = OtherPublicCertFile)
    if Verbose:
        print (Command)
...
if __name__ == '__main__':
    ...
    def DecodeJsonFileParse (Json):
        ...
        for Config in Json['Payloads']:
            ...
            OpenSslSignerPrivateKey      = ConvertJsonValue (Config, 'OpenSslSignerPrivateKey', os.path.expandvars, Required = False, Default = None, Open = True)
            OpenSslSignerPublicCertFile  = ConvertJsonValue (Config, 'OpenSslSignerPublicCertFile', os.path.expandvars, Required = False, Default = None, Open = True)
            ...
            ImageHeaderVersion           = ConvertJsonValue (Config, 'ImageHeaderVersion', ValidateUnsignedInteger, Required = False, Default = 0x00000003)
            ...
            PayloadDescriptorList.append (PayloadDescriptor (
                                            ...
                                            UpdateImageIndex,
                                            SignToolPfxFile,
                                            OpenSslSignerPrivateKey,
                                            OpenSslSignerPublicCertFile,
    ...
    def EncodeJsonFileParse (Json):
        ...
        for Config in Json['Payloads']:
            ...
            MonotonicCount               = ConvertJsonValue (Config, 'MonotonicCount', ValidateUnsignedInteger, Required = False, Default = 0)
            SignToolPfxFile              = ConvertJsonValue (Config, 'SignToolPfxFile', os.path.expandvars, Required = False, Default = None, Open = True)
            OpenSslSignerPrivateKey      = ConvertJsonValue (Config, 'OpenSslSignerPrivateKey', os.path.expandvars, Required = False, Default = None, Open = False)
            OpenSslSignerPublicCertFile  = ConvertJsonValue (Config, 'OpenSslSignerPublicCertFile', os.path.expandvars, Required = False, Default = None, Open = True)
            OpenSslOtherPublicCertFile   = ConvertJsonValue (Config, 'OpenSslOtherPublicCertFile', os.path.expandvars, Required = False, Default = None, Open = True)
            SigningToolPath              = ConvertJsonValue (Config, 'SigningToolPath', os.path.expandvars, Required = False, Default = None)
    ...
    def GenerateOutputJson (PayloadJsonDescriptorList):
                          ...
                          "Payloads" : [
                              {
                                  ...
                                  "OpenSslSignerPrivateKey": str(PayloadDescriptor.OpenSslSignerPrivateKey),
                                  "OpenSslSignerPublicCertFile": str(PayloadDescriptor.OpenSslSignerPublicCertFile),
                                  ...
                                  "ImageHeaderVersion" : str(PayloadDescriptor.ImageHeaderVersion)
        ...
        for PayloadField in PayloadSection:
            ...
            if PayloadJsonDescriptorList[Index].OpenSslSignerPrivateKey is None:
                del PayloadField ['OpenSslSignerPrivateKey']
            if PayloadJsonDescriptorList[Index].OpenSslSignerPublicCertFile is None:
                del PayloadField ['OpenSslSignerPublicCertFile']
    ...
    def CheckArgumentConflict (args):
        ...
        if args.SignToolPfxFile:
            ...
            sys.exit (1)
        if args.OpenSslSignerPrivateKey:
            print ('GenerateCapsule: error: Argument --signer-private-key conflicts with Argument -j')
            sys.exit (1)
        if args.OpenSslSignerPublicCertFile:
            print ('GenerateCapsule: error: Argument --signer-public-cert conflicts with Argument -j')
            sys.exit (1)
    ...
    class PayloadDescriptor (object):
        def __init__(self,
                     ...
                     OpenSslSignerPrivateKey      = None,
                     OpenSslSignerPublicCertFile  = None,
                     OpenSslOtherPublicCertFile   = None,
                     SigningToolPath              = None,
                     DepexExp                     = None,
                     ImageHeaderVersion           = 0x00000003
                     ):
            ...
            self.OpenSslSignerPrivateKey      = OpenSslSignerPrivateKey
            self.OpenSslSignerPublicCertFile  = OpenSslSignerPublicCertFile
            self.OpenSslOtherPublicCertFile   = OpenSslOtherPublicCertFile
            self.SigningToolPath              = SigningToolPath
            self.DepexExp                     = None if DepexExp == "None" else DepexExp
            self.ImageHeaderVersion           = ImageHeaderVersion

            self.UseSignTool = self.SignToolPfxFile is not None
            self.UseOpenSsl  = (self.OpenSslSignerPrivateKey is not None and
                                self.OpenSslSignerPublicCertFile is not None)
            self.AnyOpenSsl  = (self.OpenSslSignerPrivateKey is not None or
                                self.OpenSslSignerPublicCertFile is not None or
                                self.OpenSslOtherPublicCertFile is not None)
            self.UseDependency = self.DepexExp is not None
            self.SkipSign    = (not self.UseSignTool and not self.UseOpenSsl)
        ...
        def Validate(self, args):
            ...
            if not self.UseSignTool and not self.UseOpenSsl and self.AnyOpenSsl:
                if args.JsonFile:
                    raise argparse.ArgumentTypeError ('the following JSON fields are required for OpenSSL: OpenSslSignerPrivateKey, OpenSslOtherPublicCertFile')
                else:
                    raise argparse.ArgumentTypeError ('the following options are required for OpenSSL: --signer-private-key, --other-public-cert')
            ...
            if args.Encode:
                ...
                if self.ImageHeaderVersion > 0x00000003 or self.ImageHeaderVersion < 0x00000001:
                    if args.JsonFile:
                        raise argparse.ArgumentTypeError ('JSON field ImageHeaderVersion must be an integer in range 0x00000001..0x00000003')
                    else:
                        raise argparse.ArgumentTypeError ('--capversion must be an integer in range 0x00000001..0x00000003')

            if self.UseSignTool:
                self.SignToolPfxFile.close()
                self.SignToolPfxFile = self.SignToolPfxFile.name
            if self.UseOpenSsl:
                self.OpenSslSignerPublicCertFile.close()
                self.OpenSslSignerPublicCertFile   = self.OpenSslSignerPublicCertFile.name
                if self.OpenSslOtherPublicCertFile is not None:
    ...
    def Encode (PayloadDescriptorList, EmbeddedDriverDescriptorList, Buffer):
        if args.JsonFile:
            ...
            PayloadDescriptorList.append (PayloadDescriptor (
                                            ...
                                            args.OpenSslSignerPrivateKey,
                                            args.OpenSslSignerPublicCertFile,
                                            args.OpenSslOtherPublicCertFile,
                                            args.SigningToolPath,
                                            None,
                                            args.ImageHeaderVersion
                                            ))
        ...
        for SinglePayloadDescriptor in PayloadDescriptorList:
            ...
            if SinglePayloadDescriptor.UseOpenSsl or SinglePayloadDescriptor.UseSignTool or \
                SinglePayloadDescriptor.SkipSign:
                #
                ...
                try:
                    if SinglePayloadDescriptor.UseSignTool:
                        ...
                        CertData = SignPayloadOpenSsl (
                            Result + struct.pack ('<Q', SinglePayloadDescriptor.MonotonicCount),
                            SinglePayloadDescriptor.SigningToolPath,
                            SinglePayloadDescriptor.OpenSslSignerPrivateKey,
                            SinglePayloadDescriptor.OpenSslSignerPublicCertFile,
                            SinglePayloadDescriptor.OpenSslOtherPublicCertFile,
                            Verbose = args.Verbose,
                            Engine = args.OpenSslEngine,
                            MsgDigest = args.Digest
                        )
                    else:
                        CertData = b''
                except Exception as Msg:
                ...
                try:
                    ...
                    sys.exit (1)
            FmpCapsuleHeader.AddPayload (SinglePayloadDescriptor.Guid, Result, HardwareInstance = SinglePayloadDescriptor.HardwareInstance, UpdateImageIndex = SinglePayloadDescriptor.UpdateImageIndex, CapsuleSupport = ImageCapsuleSupport, ImageHeaderVersion = SinglePayloadDescriptor.ImageHeaderVersion)
        try:
            for EmbeddedDriver in EmbeddedDriverDescriptorList:
                FmpCapsuleHeader.AddEmbeddedDriver(EmbeddedDriver)

            Result = FmpCapsuleHeader.Encode (ImageHeaderVersion = args.ImageHeaderVersion)
            if args.Verbose:
    ...
    def Decode (PayloadDescriptorList, PayloadJsonDescriptorList, Buffer):
        ...
        try:
            ...
            if len (Result) > 0:
                Result = FmpCapsuleHeader.Decode (Result, args.ImageHeaderVersion)
                if args.JsonFile:
                    ...
                    for Index in range (0, FmpCapsuleHeader.PayloadItemCount):
                        if Index < len (PayloadDescriptorList):
                            ...
                            PayloadJsonDescriptorList.append (PayloadDescriptor (
                                                                ...
                                                                PayloadDescriptorList[Index].OpenSslSignerPrivateKey,
                                                                PayloadDescriptorList[Index].OpenSslSignerPublicCertFile,
                                                                PayloadDescriptorList[Index].OpenSslOtherPublicCertFile,
                                                                PayloadDescriptorList[Index].SigningToolPath,
                                                                None,
                                                                PayloadDescriptorList[Index].ImageHeaderVersion
                                                                ))
                else:
    ...
    def DumpInfo (Buffer, args):
        ...
        try:
            ...
            UefiCapsuleHeader.DumpInfo ()
            if len (Result) > 0:
                FmpCapsuleHeader.Decode (Result, args.ImageHeaderVersion)
                print ('--------')
                FmpCapsuleHeader.DumpInfo ()
                for Index in range (0, FmpCapsuleHeader.PayloadItemCount):
    ...
    parser.add_argument ("--capversion", dest = 'ImageHeaderVersion', type = ValidateUnsignedInteger, default = 0x00000003,
                         help = "The 32-bit version of the FMP capsule image header.  The default is 0x00000003.")
    ...
    parser.add_argument ("--signer-private-key", dest='OpenSslSignerPrivateKey',
                         help="OpenSSL signer private key filename or PKCS11 URI.")
    parser.add_argument ("--signer-public-cert", dest='OpenSslSignerPublicCertFile', type=argparse.FileType('rb'),
                         help="OpenSSL signer public certificate filename.")
    ...
    parser.add_argument ("--engine", dest='OpenSslEngine', help="OpenSSL engine name.")

    parser.add_argument ("--digest", dest='Digest',
                         help="Message digest algorithm to use when signing. Valid when payload is signed using OpenSSL only. The default is sha256.")
